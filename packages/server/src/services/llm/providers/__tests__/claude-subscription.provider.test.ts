...
describe("ClaudeSubscriptionProvider — resume path wiring", () => {
  ...
  it("lifts the trailing assistant prefill out of history and sends a synthetic continuation prompt", async () => {
    const captured = installFakeSdk();
    ...
    const call = captured[0]!;
    // The prefill assistant turn is lifted OUT of the resume transcript and
    // carried in the synthetic continuation prompt instead (see
    // buildAssistantPrefillContinuationPrompt); only the prior user turn remains.
    assert.ok(call.resumeEntries, "sessionStore.load() should have returned entries");
    const entries = call.resumeEntries!;
    assert.equal(entries[entries.length - 1]!["type"], "user", "prior user turn is the tail of the transcript");
    assert.ok(
      !entries.some((entry) => entry["type"] === "assistant"),
      "prefill assistant no longer rides the transcript",
    );
