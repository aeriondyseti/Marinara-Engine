...
describe("splitHistoryForResume", () => {
  ...
  it("lifts a trailing assistant prefill into a synthetic continuation prompt (prefill path)", () => {
    // Marinara's assistantPrefill feature lands here (generate.routes.ts). The
    // trailing assistant is the prefill: it's removed from the JSONL history and
    // its text is carried in a synthetic user continuation prompt instead.
    const history: ChatMessage[] = [
    ...
    // Prefill no longer rides the JSONL; only the prior user turn remains.
    assert.equal(split.history.length, 1);
    assert.equal(split.history[0]!.role, "user");
    assert.equal(split.history[0]!.content, "story start: ");
    // The prefill text is carried in the synthetic continuation prompt instead.
    assert.equal(split.current.role, "user");
    assert.match(split.current.content, /Once upon a time,/);
    assert.ok(split.current.content.length > 0, "synthetic continuation must be non-empty for the Anthropic API");
    assert.equal(split.assistantPrefillLength, "Once upon a time,".length);
  });
