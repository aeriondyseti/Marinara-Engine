// ──────────────────────────────────────────────
// Modal: Promote NPC to Character Card
// Samples dialogue, avatar, and NPC description
// to generate a complete character card via AI
// ──────────────────────────────────────────────
...
import type { GameNpc, Message } from "@marinara-engine/shared";
...
export function PromoteNpcToCharacterModal({ open, onClose, npc, chatMessages }: Props) {
  ...
  // Sample dialogue involving this NPC from chat history
  const dialogueSample = useMemo(() => {
    if (!npc || !chatMessages.length) return "";

    const npcNameLower = npc.name.toLowerCase();
    const relevantMessages: string[] = [];

    // Look for messages that mention the NPC (in content or as speaker)
    for (const msg of chatMessages) {
      const content = msg.extra?.displayText || msg.content || "";
      const contentLower = content.toLowerCase();

      // Include messages that mention the NPC or were spoken by them
      if (contentLower.includes(npcNameLower)) {
        // Format as dialogue if possible
        if (msg.role === "assistant") {
          relevantMessages.push(`${npc.name}: ${content.slice(0, 200)}`);
        } else if (msg.role === "user") {
          relevantMessages.push(`Player: ${content.slice(0, 200)}`);
        } else {
          relevantMessages.push(content.slice(0, 200));
        }

        // Limit to ~5-8 exchanges
        if (relevantMessages.length >= 8) break;
      }
    }
  ...
  // Generate avatar description from URL or placeholder
  const avatarDescription = useMemo(() => {
    if (!npc?.avatarUrl) return "No avatar image available.";
    // For now, use a simple description. In the future, this could be enhanced
    // with vision model analysis of the actual image.
    return `Avatar image available: ${npc.avatarUrl}`;
  }, [npc?.avatarUrl]);

  // Build the enriched prompt for character generation
  const enrichedPrompt = useMemo(() => {
    if (!npc) return "";

    const parts: string[] = [];
    parts.push(`Create a complete character card for an NPC from an ongoing game session:\n`);
    ...
    if (npc.gender || npc.pronouns) {
      const gender = npc.gender ? `Gender: ${npc.gender}` : "";
      const pronouns = npc.pronouns ? `Pronouns: ${npc.pronouns}` : "";
      parts.push(`\n${[gender, pronouns].filter(Boolean).join(", ")}`);
    }

    if (npc.location) {
      parts.push(`\nLast Known Location: ${npc.location}`);
    }

    if (npc.notes && npc.notes.length > 0) {
      parts.push(`\nPlayer Notes & Interactions:\n${npc.notes.join("\n")}`);
    }
    ...
    parts.push(
      `\nBased on this information, generate a complete, reusable character card with personality, backstory, scenario, first message, and system prompt. Make the character feel like the established NPC from the game session.`,
    );

    return parts.join("\n");
  }, [npc, dialogueSample, avatarDescription]);

  const handleGenerate = useCallback(async () => {
    if (!enrichedPrompt.trim() || !connectionId) return;
  ...
  }, [enrichedPrompt, connectionId, enableStreaming]);
  ...
  return (
    <Modal open={open} onClose={handleClose} title={`✦ Promote ${npc.name} to Character Card`} width="max-w-lg">
      ...
      <div className="space-y-4">
        ...
        {/* Preview of extracted NPC data */}
        <div className="rounded-xl border border-[var(--border)] bg-[var(--secondary)]/30 p-3 space-y-2 text-xs">
          <div className="font-medium text-[var(--muted-foreground)]">Extracting NPC Data:</div>
          {npc.description && (
            <div className="text-[var(--foreground)]/80 line-clamp-2">
              <span className="text-[var(--muted-foreground)]">Description:</span> {npc.description}
            </div>
          )}
          {dialogueSample && (
            <div className="text-[var(--foreground)]/80 line-clamp-2">
              <span className="text-[var(--muted-foreground)]">Dialogue:</span> {dialogueSample.split("\n")[0]}
            </div>
          )}
          {npc.notes && npc.notes.length > 0 && (
            <div className="text-[var(--foreground)]/80 line-clamp-2">
              <span className="text-[var(--muted-foreground)]">Notes:</span> {npc.notes[0]}
            </div>
          )}
          {npc.avatarUrl && (
            <div className="text-[var(--foreground)]/80">
              <span className="text-[var(--muted-foreground)]">Avatar:</span> Available for analysis
            </div>
          )}
        </div>
        ...
        <button
          ...
          disabled={streaming || !connectionId}
